...
class TSTransformerEncoder(nn.Module):
    ...
    def __init__(self, vocab_size, feat_dim=512, max_len=128, d_model=64, n_heads=8, num_layers=3, dim_feedforward=256, dropout=0.1,
...
class LogKeyFeatureExtrator(nn.Module):
    ...
    def forward(self, inputs, padding_masks):
        output = self.model(inputs, padding_masks)
        output = torch.einsum('bsf->bf', [output])
        feature = self.linear(output)
